...
serve(async (req) => {
  ...
  try {
    ...
    if (error || !row) {
      return new Response(JSON.stringify({ error: "No pending verification found. Please sign up again." }), {
        status: 404, headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }

    if (new Date(row.expires_at).getTime() < Date.now()) {
      await admin.from("signup_otps").delete().eq("email", lowerEmail);
    ...
    const p = row.pending_data as {
      password: string; fullName: string; userType: string;
    ...
    const { data: created, error: createErr } = await admin.auth.admin.createUser({
      ...
      user_metadata: {
        ...
        ...(p.institutionName ? { institution_name: p.institutionName } : {}),
