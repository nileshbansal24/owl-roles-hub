...
serve(async (req) => {
  ...
  try {
    ...
    const { email, password, fullName, userType, institutionName, designation } = body ?? {};
    ...
    const { error: upsertErr } = await admin.from("signup_otps").upsert({
      email: lowerEmail,
      code_hash,
      pending_data: { password, fullName, userType, institutionName, designation },
      attempts: 0,
      expires_at,
    ...
    if (upsertErr) {
      ...
      return new Response(JSON.stringify({ error: "Could not create OTP" }), {
        status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
      ...
      <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;background:#f9fafb;padding:40px 20px;">
        <div style="max-width:520px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 1px 3px rgba(0,0,0,.08);">
          ...
          <div style="padding:32px 28px;color:#111827;">
            ...
            <p style="margin:0 0 20px;font-size:14px;color:#4b5563;">Use this 6-digit code to verify your email and finish creating your account:</p>
    ...
    const emailRes = await fetch(`${GATEWAY_URL}/v3/smtp/email`, {
      ...
      body: JSON.stringify({
        ...
        subject: `${code} is your OWL ROLES verification code`,
        htmlContent: html,
      }),
    });
    ...
    if (!emailRes.ok) {
      ...
      return new Response(JSON.stringify({ error: "Could not send verification email. Please try again." }), {
