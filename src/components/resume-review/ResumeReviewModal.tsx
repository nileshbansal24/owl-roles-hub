...
import { ParsedResumeData, ParsedExperience, ParsedEducation, ParsedResearchPaper } from "./types";
...
export function ResumeReviewModal({
  ...
  const removeResearchPaper = (index: number) => {
    updateField("research_papers", (data.research_papers || []).filter((_, i) => i !== index));
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[90vh] p-0">
        <DialogHeader className="px-6 pt-6 pb-2">
          ...
          <DialogDescription>
            Review and edit the information extracted from your resume before saving to your profile.
          </DialogDescription>
        </DialogHeader>
          ...
          <Tabs defaultValue="basic" className="w-full">
                ...
                <div className="space-y-2 mb-2">
                  {(data.achievements || []).map((achievement, index) => (
                    <div key={index} className="flex items-start gap-2 p-2 bg-muted rounded-md">
                      ...
                      <button onClick={() => removeAchievement(index)} className="text-muted-foreground hover:text-destructive">
                        <X className="h-4 w-4" />
                      </button>
                    </div>
                  ))}
            ...
            <TabsContent value="experience" className="space-y-4">
              {(data.experience || []).length === 0 ? (
                <p className="text-muted-foreground text-center py-8">No experience extracted from resume.</p>
              ) : (
                  ...
                  <div key={index} className="border rounded-lg p-4 space-y-3 relative">
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      ...
                      <div className="space-y-1">
                        ...
                        <Input value={exp.start_date || ""} onChange={(e) => updateExperience(index, "start_date", e.target.value)} />
                      </div>
                      <div className="space-y-1">
                        <Label className="text-xs">End Date</Label>
                        <Input value={exp.end_date || ""} onChange={(e) => updateExperience(index, "end_date", e.target.value)} placeholder={exp.current ? "Present" : ""} />
                      </div>
                    </div>
                    <div className="space-y-1">
            ...
            <TabsContent value="education" className="space-y-4">
              {(data.education || []).length === 0 ? (
                <p className="text-muted-foreground text-center py-8">No education extracted from resume.</p>
              ) : (
            ...
            <TabsContent value="research" className="space-y-4">
              {(data.research_papers || []).length === 0 ? (
                <p className="text-muted-foreground text-center py-8">No research papers extracted from resume.</p>
              ) : (
                (data.research_papers || []).map((paper, index) => (
                  <div key={index} className="border rounded-lg p-4 space-y-3 relative">
                    <button onClick={() => removeResearchPaper(index)} className="absolute top-2 right-2 text-muted-foreground hover:text-destructive">
                      <X className="h-4 w-4" />
                    </button>
                    <div className="space-y-1">
                      <Label className="text-xs">Title</Label>
                      <Input value={paper.title || ""} onChange={(e) => updateResearchPaper(index, "title", e.target.value)} />
                    </div>
                    <div className="grid grid-cols-3 gap-3">
                      <div className="space-y-1">
                        <Label className="text-xs">Journal</Label>
                        <Input value={paper.journal || ""} onChange={(e) => updateResearchPaper(index, "journal", e.target.value)} />
                      </div>
                      <div className="space-y-1">
                        <Label className="text-xs">Year</Label>
                        <Input value={paper.year || ""} onChange={(e) => updateResearchPaper(index, "year", e.target.value)} />
                      </div>
                      <div className="space-y-1">
                        <Label className="text-xs">DOI</Label>
                        <Input value={paper.doi || ""} onChange={(e) => updateResearchPaper(index, "doi", e.target.value)} placeholder="10.xxxx/xxxxx" />
                      </div>
                    </div>
                    <div className="space-y-1">
                      <Label className="text-xs">Authors</Label>
                      <Input value={paper.authors || ""} onChange={(e) => updateResearchPaper(index, "authors", e.target.value)} />
                    </div>
                  </div>
                ))
              )}
